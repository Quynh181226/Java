...
import JavaAdvanced.Project_JavaAdvanced.SmartPhoneStore.src.dao.imps.CouponProductDAOImpl;
...
public class CouponService {
    ...
    public CouponService() {
        ...
        this.couponProductDAO = new CouponProductDAOImpl();
    }
    ...
    public boolean isCouponApplicableToCart(String code, Map<Integer, CartService.CartItem> cart) throws SQLException {
        Coupon coupon = couponDAO.findByCode(code);
        if (coupon == null) {
            return false;
        }

        String applicableType = coupon.getApplicableType();
        if ("ALL".equals(applicableType)) {
            return true;
        }

        Set<Integer> allowedProductIds = couponProductDAO.getProductIdsByCoupon(coupon.getId());
        Set<String> allowedBrands = couponProductDAO.getBrandsByCoupon(coupon.getId());

        for (CartService.CartItem item : cart.values()) {
            ...
            if (allowedBrands.contains(product.getBrand())) {
                applicable = true;
            }

            if (!applicable && !"ALL".equals(applicableType)) {
                return false;
            }
        }
